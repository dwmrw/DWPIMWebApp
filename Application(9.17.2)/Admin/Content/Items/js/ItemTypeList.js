...
Dynamicweb.Items.ItemTypeList.onShow = function (rowId) {
    const row = List.getRowByID('lstItemTypes', rowId);
    const href = document.querySelector('#cmItem #cmiCustomize a');
    if (row.dataset.customizable) {
        href.classList.remove('hidden');
    } else {
        href.classList.add('hidden');
    }
};
