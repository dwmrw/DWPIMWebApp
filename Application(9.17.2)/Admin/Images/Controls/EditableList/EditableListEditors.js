...
Dynamicweb.Controls.EditableList.Editors.ListEditor.prototype.format = function (value) {
    ...
    for (var i = 0; i < this._options.length; i++) {
        var optionCandidate = this._options[i];
        if (optionCandidate.items) {
            for (var j = 0; j < optionCandidate.items.length; j++) {
                if (optionCandidate.items[j].value === value) {
                    option = optionCandidate.items[j];
                    break;
                }
            }
        } else if (optionCandidate.value === value) {
    ...
    if (!this._initialized) {
        ...
        this._control = {
            ...
            formatDate: function (dt, frm) {
                if (!dt) {
                    return document.getElementById('DWCalendarNotSetString').value;
                }
