        ...
        if (Array.isArray(defs[0])) {
            ...
            for (var i = 0; i < defs.length; i++) {
                for (var j = 0; j < defs[i].length; j++) {
                    controlDefinitions[indexer] = defs[i][j];
                    indexer++;
                }
            }
        } else {
            controlDefinitions = controlDefinitions.concat(defs);
        }
    }
...
Dynamicweb.PIM.BulkEdit.prototype.getFieldByControlDefinition = function (controlDefinition) {
    ...
    switch (controlDefinition.controlType) {
        ...
        case "MultiSelectList":
        case "Select":
            field = document.getElementById(controlDefinition.controlId);
            break;
        ...
        case "Filemanager":
            field = document.getElementById(controlDefinition.controlId).next(".box-control-box");
            break;
    }
    return field;
};
...
Dynamicweb.PIM.BulkEdit.prototype._enableBulkEditRibbonButton = function () {
    ...
    if (self.options.permissions.GridEditingPermission >= (PermissionLevels.Edit || PermissionLevels.PermissionLevelEdit)) {
...
Dynamicweb.PIM.BulkEdit.prototype._enableExportToExcelRibbonButton = function () {
    var self = this;
    if ((self._queryId || self._groupId) && self.options.permissions.ExportExcelPermission >= (PermissionLevels.Read || PermissionLevels.PermissionLevelRead)) {
...
Dynamicweb.PIM.BulkEdit.prototype.openProduct = function ({ productId, variantId = "", queryId, groupId, scrollToVariant = false, scrollToField = false, fieldControlIdToScroll, listPageIds = null, showAllProducts = undefined }) {
    var self = this;
    var dynamicStructure = self._dynamicStructure;
    var url = self.getProductEditUrl({ productId, variantId, queryId, groupId, dynamicStructure  });
    window.top.navigateToVariants = false;
    window.top.fieldControlIdToScroll = null;
    this.setScrollCoordinates(null);
    if (scrollToVariant) {
...
Dynamicweb.PIM.BulkEdit.prototype.getProductEditUrl = function ({ productId, variantId = "", queryId, groupId, dynamicStructure }) {
    var self = this;
    ...
    if (queryId) {
        ...
        if (self._groupId) {
            parameters += "&groupID=" + self._groupId;
        }
        if (dynamicStructure) {
            parameters += "&DynamicStructure=" + dynamicStructure;
        }
    } else {
...
Dynamicweb.PIM.BulkEdit.prototype.productFieldsBulkEdit = function (productId) {
    ...
    if (self._basePath) {
        url += "&BasePath=" + self._basePath;
    }
    self.openScreenAction.Url = url;
    Action.Execute(self.openScreenAction, {
        productId: productId,
