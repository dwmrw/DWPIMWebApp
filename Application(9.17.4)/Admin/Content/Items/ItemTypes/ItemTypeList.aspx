...
<head runat="server">
    ...
    <script type="text/javascript">
        ...
        function customizeItemType(rowId, systemName) {
            const action = <%=GetCustomizeItemTypeAction()%>;
            const row = List.getRowByID('lstItemTypes', rowId);
            if (!action || !row.dataset.customizable) {
                return;
            }

            const submitAction = action.OnSubmitted.Actions[0];
            const systemNameIndex = submitAction.Url.lastIndexOf('=') + 1;
            submitAction.Url = submitAction.Url.substring(0, systemNameIndex) + systemName;
...
<body class="screen-container">
    ...
    <dw:ContextMenu ID="cmItem" runat="server" OnClientSelectView="Dynamicweb.Items.ItemTypeList.onContextMenuView" OnShow="Dynamicweb.Items.ItemTypeList.onShow(ContextMenu.callingID)">
        ...
        <dw:ContextMenuButton ID="cmiCustomize" Views="mixed,common" Icon="ModeEdit" Text="Customize" OnClientClick="customizeItemType(ContextMenu.callingID, ContextMenu.callingItemID);" runat="server" />
