...
function updateTNTDialog(dialogId, parentID) {
    ...
    var parameterArr = inputs.map(function (input) {
        return input.value;
    }).join(',');
...
function openCaptureDialog() {    
    ...
    dialog.set_cancelButtonOnclick("CaptureDialog", reloadFn);
    var dialogCloseIcon = $$("#H_CaptureDialog .close>i")[0];
    dialogCloseIcon.onclick = reloadFn;
}
...
function changeViewToEdit() {
    Ribbon.activateButton('EditOrderButton');
    Ribbon.deactivateButton('DetailsButton');
    changeView(1);
    ...
    $$("input[id^='OlUnitPrice']").each(function (e) {
        ...
        e.style.borderStyle = 'solid';
        e.show();
        e.parentNode.querySelector(".js-nice-price")?.hide();
    });
...
function changeViewToDetails() {
    Ribbon.deactivateButton('EditOrderButton');
    Ribbon.activateButton('DetailsButton');
    changeView(1);
    ...
    $$("input[id^='OlUnitPrice']").each(function (e) {
        ...
        e.style.borderStyle = 'none';
        e.hide();
        e.parentNode.querySelector(".js-nice-price")?.show();
    });
...
function ValidateQuantity(event) {
    ...
    if (key != 35 && key != 36 && key != 37 && key != 38 && key != 39 && key != 40 && key != 46 && key != 8 && key != 44) {
...
function SetUnitPrice(unitPriceWithVatId, unitPriceId, VAT, decimalPlaces) {
    const olUnitPriceWithVat = document.getElementById(unitPriceWithVatId).value;
    const useComma = olUnitPriceWithVat.indexOf(',') !== -1;
    const value = parseFloat(olUnitPriceWithVat) * VAT;

    document.getElementById(unitPriceId).value = formatFloatValue(value, decimalPlaces, useComma);
}

function SetUnitPriceWithVat(unitPriceWithVatId, unitPriceId, VAT, decimalPlaces) {
    const olUnitPrice = document.getElementById(unitPriceId).value;
    const useComma = olUnitPrice.indexOf(',') !== -1
    const value = parseFloat(olUnitPrice) / VAT;

    document.getElementById(unitPriceWithVatId).value = formatFloatValue(value, decimalPlaces, useComma);
}

function formatFloatValue(value, decimalPlaces, useComma) {
    if (isNaN(value)) {
        return "";
    }

    let result = value;
    if (result % 1 !== 0) { // Is float
        result = value.toFixed(decimalPlaces); // The .toFixed method is always use '.' as decimal separator
        if (useComma) {
