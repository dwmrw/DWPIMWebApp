var mails = new Array();
function Mail(recipient, subject, template, templatePath, senderName, senderMail, isCustomer, isDelivery, encodingCodePage, attachment, attachmentPath, isCustomField, recipientCustomField, redirectType, redirectPage) {
...
function editMail(mailIndex) {
    ...
    document.querySelector(`input[type='radio'][name='RedirectType'][value='${mail.RedirectType}']`).click();
    document.getElementById('Link_RedirectPage').value = mail.RedirectPage;
    document.getElementById('RedirectPage').value = mail.RedirectPage;
...
function saveMailEdit() {
    ...
    mail.RedirectType = document.querySelector('[name="RedirectType"]:checked').value;
    mail.RedirectPage = document.getElementById('RedirectPage').value;
...
function addNewMail() {
    mails.push(new Mail('', '', '', '', '', '', false, false, 65001, '', '', false, '', 'Template', '')); // 65001 is the codepage for UTF-8
    updateMails();
...
function updateMails() {
    ...
    for (var i = 0; i < mails.length; i++) {
        ...
        addHidden('Mails', 'Mail' + (i + 1) + 'RedirectType', mail.RedirectType);
        addHidden('Mails', 'Mail' + (i + 1) + 'RedirectPage', mail.RedirectPage);
